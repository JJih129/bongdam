...
module.exports = async (h) => {
  ...
  const d = await h.page.evaluate(() => {
    const t0 = Number(localStorage.getItem('bd_play_started') || 0);
    return { stamped: !!t0, ageSec: t0 ? Math.round((Date.now() - t0) / 1000) : null, wrapped: !!(window.BD_confirmStartSetup && BD_confirmStartSetup.__v313) };
  });
  say('플레이 시간 기록: ' + JSON.stringify(d));
  say(d.stamped && d.ageSec < 60 ? '✅ 통과' : '❌ 확인 필요');
};
