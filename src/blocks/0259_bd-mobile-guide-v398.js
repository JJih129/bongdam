...
(function () {
  ...
  var MAP = [
    ...
    [/\s*\(\s*ESC\s*\)\s*\./g, '.'],   /* (v399) «괜찮아요 (ESC).» → «괜찮아요.» — 괄호만 지우면 마침표 앞에 공백이 남았다 */
    [/\(\s*ESC\s*\)/g, ''],
    ...
    [/<?\s*Spacebar\s*>?/g, '화면 탭'],
    [/Space를/g, '화면을'],
    [/F로 조사하세요/g, '조사 버튼을 눌러요'], [/F로 조사/g, '조사 버튼으로 조사'], [/F 키/g, '조사 버튼'],   /* (v399e 검수) 모바일에 남던 키보드 «F» 안내 */   /* (v399) «그때 Space를 눌러요» → «그때 화면을 눌러요» — \b 는 한글 앞에서 경계가 안 잡혀 «화면 탭를»이 됐다 */
    [/\bSpace\b/g, '화면 탭'],
    [/\bESC\b/g, '닫기']
  ];
